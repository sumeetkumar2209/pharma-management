...
@Service
public class ModifyRejectedProduct implements ModifyProduct {
    @Autowired
    ...
    public String modifyProduct(ProductDTO productDTO) throws RecordNotFoundException, InvalidStatusException {
        ...
        if(productDOIntPending != null && !productDOIntPending.getReviewStatus().getReviewCode().equalsIgnoreCase("RE")){
            throw new InvalidStatusException("Already pending modification");
        } else if (productDOIntPending == null) {
            throw new RecordNotFoundException("Record not found error");
        }
        ProductDO_INT productDOInt = context.getBean(ProductDO_INT.class);
        BeanUtils.copyProperties(productIntRepo, productDOInt);
