...
@Service
public class ModifyRejectedCustomer implements ModifyCustomer {
    ...
    public void modifyCustomer(CustomerDTO customerDTO) throws RecordNotFoundException, InvalidStatusException {
        ...
        if(customerDOIntPending != null && !customerDOIntPending.getReviewStatus().getReviewCode().equalsIgnoreCase("RE"))
        {
            throw new InvalidStatusException("Already pending modification");
        } else if (customerDOIntPending == null) {
        ...
        BeanUtils.copyProperties(customerDOIntPending, customerDOInt);
